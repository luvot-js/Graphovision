...
기본: 룰 기반 템플릿 (Claude API 키 없어도 동작)
선택: ANTHROPIC_API_KEY 환경변수 설정 시 Claude API 사용
"""
...
def _claude_report(scores: list[float]) -> str:
    try:
        import anthropic
        client = anthropic.Anthropic()

        trait_lines = "\n".join(
            f"- {name}: {score:.0%}" for name, score in zip(TRAIT_NAMES_KR, scores)
        )
        prompt = (
            "다음은 필기 분석을 통해 도출된 성격 지표 점수입니다:\n\n"
            f"{trait_lines}\n\n"
            "이 점수를 바탕으로 150자 내외의 한국어 성격 분석 리포트를 작성해주세요. "
            "점수가 높을수록 그 특성이 강합니다. 존댓말로 작성하세요."
        )
